...
> **Note:** `VehicleLocationsResource` does not declare a typed `$entity`. Each result is a generic `Entity` (a `Fluent` instance) keyed by the response shape documented in [Available Properties](#available-properties). There are no `getLatitude()` / `getLongitude()` helpers on the result; access keys directly with `$location->latitude`.
