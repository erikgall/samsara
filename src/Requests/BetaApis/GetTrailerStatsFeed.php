 ...
 *
 * The first call to this endpoint will provide the most recent stats
 * for each trailer and an `endCursor`.
 *
 * Providing the `endCursor` value to the `after` query parameter
 * will fetch all updates since the previous API call.
 *
 * If `hasNextPage` is false, no new data is
 * immediately available. Please wait a minimum of 5 seconds before making a subsequent request.
 *
 *
 ...
 */
class GetTrailerStatsFeed extends Request
{
    ...
    {
        return '/beta/fleet/trailers/stats/feed';
    }
}
