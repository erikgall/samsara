...
use ErikGall\Samsara\Requests\VehicleStats\GetVehicleStats;
use ErikGall\Samsara\Requests\VehicleStats\GetVehicleStatsFeed;
use ErikGall\Samsara\Requests\VehicleStats\GetVehicleStatsHistory;

class VehicleStats extends Resource
{
    /**
     ...
     * - `spreaderPlowStatus`: Snow plow status (`Up` or `Down`), as read from the material spreader. Note: this is separate from plow status defined via auxInput.
     * - `seatbeltDriver`: Seatbelt Driver Status as read from the vehicle. `Buckled` or `Unbuckled`. (Beta only)
     * @param  array  $decorations  Decorations to add to the primary stats listed in the `types` parameter. For example, if you wish to know the vehicle's location whenever the engine changes state, you may set `types=engineStates&decorations=gps`.
     *
